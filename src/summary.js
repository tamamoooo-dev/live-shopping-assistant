...
export function summaryElement(s, storeLabelFn = (x) => x, opts = {}) {
  ...
  const discountIndicator = discountDot(h.offer || h.it || h);
  if (discountIndicator) head.appendChild(discountIndicator);
  // Discount depth: the colored dot alone (the percentage survives as a tooltip).
